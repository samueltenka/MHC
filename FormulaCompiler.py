...
class FormulaParser(Lexer, Generator):
   ...
   def __init__(self, text):
        ...
        self.ES = ExprStack.ExprStack(framecapacityreg=1, stackreg=2, workregs=[3,4,5,6])
        self.varmems = {'IN':0, 'OUT':1}
   ...
   def eat_atom(self):
      if self.peek()=='(':
          ...
          self.ES.push(); a = self.ES.top()
          if self.peek() not in self.varmems:
              self.varmems[self.peek()] = 'mem'+str(self.peek())
          print('load 0 %d %s' % (a,self.varmems[self.peek()]))
   ...
   def eat_product(self):
      ...
      if self.peek() == '*':
          self.eat('*')
          self.eat_product()
          a = self.ES.top(); self.ES.pop()
          b = self.ES.top(); self.ES.pop()
          self.ES.push(); d = self.ES.top();
          print('mul %d %d %d' % (a, b, d))
      elif self.peek()=='/':
          self.eat('/')
          self.eat_product()
          a = self.ES.top(); self.ES.pop()
